#Force brute, explore toutes les combianaisons
...
def Combinaison(actions, donnees, debut, fin, courant, k):
    if (courant == k):
    ...
    while (i <= fin and fin - i + 1 >= k - courant):
        ...
        Combinaison(actions, donnees, i + 1, fin, courant + 1, k)
        i += 1
        # while actions["name"][i] == actions["name"][i + 1]:
        #     i += 1
...
for k in range(1, 21):  # for k in range(1, len(actions["name"]))):
    donnees = [0] * k
    print(k)
    Combinaison(actions, donnees, 0, n - 1, 0, k)


# Step 1: Remove elements where budget is strictly greater than 500
budget_over_500_indices = [i for i, budget in enumerate(final_data['budget']) if budget > 500]

print("t1")
# Créer un dictionnaire pour stocker les données filtrées
# filtered_data = {key: [] for key in final_data.keys()}
# # Remove the elements
# for key in final_data.keys():
#     final_data[key] = [final_data[key][i] for i in range(len(final_data[key])) if i not in budget_over_500_indices]
print("t2")
# Filtrer les données et stocker dans le dictionnaire
print(len(final_data['profit']))
# for i in range(len(final_data['profit'])):
#     if i % 10000 == 0:  # Vérifie si i est un multiple de 10,000
#         print(i)
#     if i not in budget_over_500_indices:
#         for key in final_data.keys():
#             filtered_data[key].append(final_data[key][i])
# print("t3")
# Utiliser les données filtrées pour la suite du programme
...
sorted_indices = sorted(range(len(final_data['profit'])), key=lambda k: final_data['profit'][k], reverse=True)
...
# Créer un fichier csv aec tous les résultats
