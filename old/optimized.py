...
with open(chemin_fichier1, 'r') as fichier1, open(chemin_fichier2, 'r') as fichier2:
    ...
    for action in lecteur_csv1:
        if (float(action["price"]) > 0):
            donnees_fichier1.append([action["name"], float(action["price"]), float(action["profit"])])

    for action in lecteur_csv2:
        if (float(action["price"]) > 0):
            donnees_fichier2.append([action["name"], float(action["price"]), float(action["profit"])])

# Fusionner les données dans un seul tableau
actions = donnees_fichier1 + donnees_fichier2

#supprimer celle qui sont en dessous de la rentbilité moyenne
rentability = []
for action in actions:
    action_rentability = action[2] / action[1]
    rentability.append(action_rentability)
print(mean(rentability))

actions = [action for action in actions if action[2] / action[1] >= mean(rentability)]
actions = actions[0:20]
# trier les actions en fonction du prix(plus grand au plus petit)
actions = sorted(actions, key=lambda x: x[2], reverse=True)
...
def Combinaison(actions, donnees, debut, fin, courant, k, initial_maximum_budget):
    if (courant == k):
        ...
        for j in range(k):
            if (price_itm + donnees[j][1] > initial_maximum_budget):
                return
